...
if [ $? ! -ne 0 ]
    ...
    if [ $? ! -ne 0 ] then echo "error commit Publishers.xsl"; exit 1 fi
    git tag -a "$version" -m "release version $version"
    if [ $? ! -ne 0 ] then echo "error creating tag $version"; exit 1 fi
    git push origin $version
    if [ $? ! -ne 0 ] then echo "error pushing tag $version"; exit 1 fi
fi
